...
export function useLanguageTheme() {
  ...
  // Read from storage once on mount (SSR safe — always starts light/en)
  React.useEffect(() => {
    setThemeState(readStored<Theme>(THEME_KEY, "light"));
    setLangState(readStored<Lang>(LANG_KEY, "en"));
  }, []);
  ...
  const toggleLang = React.useCallback(() => {
    setLangState((prev) => {
      ...
      applyLang(next);
      dispatchChange({ theme: readStored<Theme>(THEME_KEY, "light"), lang: next });
      return next;
    });
  }, []);
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          color: isDark ? "#fff" : isDark ? "rgba(255,255,255,0.6)" : "rgba(15,30,58,0.5)",
          transition: "background 0.2s, color 0.2s, transform 0.2s",
        }}
        onMouseEnter={(e) => {
          if (!isDark) e.currentTarget.style.background = "rgba(249,115,22,0.12)";
      ...
      >
        {isDark ? <Moon size={16} strokeWidth={2.5} /> : <Sun size={16} strokeWidth={2.5} color="rgba(15,30,58,0.55)" />}
      </button>
    </div>
  );
}
