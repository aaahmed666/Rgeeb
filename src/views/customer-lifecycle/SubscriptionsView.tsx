...
const AI_STATUS_STYLES: Record<string, string> = {
  ...
  Throttled: "bg-amber-500/10 text-amber-600 dark:text-amber-400",
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 max-w-[1440px] mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-foreground">{data?.planName || "Enterprise Ultra"}</h2>
        ...
        <div className="flex gap-3 items-center w-full sm:w-auto">
          <div className="flex flex-col items-end">
            <Badge
              variant="outline"
              className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800 text-[10px] font-bold uppercase px-3 py-1 rounded-full flex items-center gap-1 shrink-0"
            >
              <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
              ACTIVE
            </Badge>
            <span className="text-[10px] text-muted-foreground mt-1 font-medium">Renewal: {data?.renewalDate}</span>
          </div>
        ...
        <div className="col-span-12 lg:col-span-8 space-y-6">
          ...
          <Card className="border border-border shadow-sm">
            <CardHeader className="pb-3 flex flex-row items-center justify-between">
              <CardTitle className="text-base font-semibold text-foreground">Service Utilization Overview</CardTitle>
              ...
              {data?.utilization.map((item) => (
                <div key={item.name} className="space-y-2">
                  ...
                  <div className="h-3 w-full overflow-hidden rounded-full bg-muted">
                    <div
                      className={cn(
                        ...
                        item.percentage >= 80 ? "bg-orange-500" : "bg-primary"
                      )}
                  ...
                  <div className="flex justify-between items-center text-[10px] font-semibold text-muted-foreground">
                    ...
                    <span className={cn(item.percentage >= 80 ? "text-amber-500" : "text-emerald-500")}>
                      {item.status}
                    </span>
                  </div>
                </div>
          ...
          <DataTable
            ...
            title="Recent AI Service Logs"
            emptyMessage="No service logs available"
          />
        </div>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6">
          ...
          <Card className="bg-primary text-primary-foreground border border-border shadow-md relative overflow-hidden">
            <CardContent className="p-6 flex flex-col items-center relative z-10">
              <h3 className="text-white text-sm font-semibold mb-6 text-center">Global Node Coverage</h3>
              {/* Circular Gauge */}
              <div className="relative w-36 h-36">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-3xl font-bold text-white">{data?.nodeCoverage.percentage}%</span>
                  <span className="text-[9px] uppercase tracking-wider text-white/60">Fleet Active</span>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4 w-full mt-6">
                <div className="bg-white/10 rounded-lg p-3 text-center border border-white/5">
                  <p className="text-[9px] text-white/60 uppercase font-semibold">Primary Nodes</p>
                  <p className="text-lg font-bold text-white mt-0.5">{data?.nodeCoverage.primaryNodes}</p>
                </div>
                <div className="bg-white/10 rounded-lg p-3 text-center border border-white/5">
                  <p className="text-[9px] text-white/60 uppercase font-semibold">Edge Gateways</p>
                  <p className="text-lg font-bold text-white mt-0.5">{data?.nodeCoverage.edgeGateways}</p>
                </div>
              </div>
            </CardContent>
          ...
          <Card className="border border-border shadow-sm p-6">
            <h3 className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest mb-4">Storage Allocation</h3>
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <div className="w-1 h-10 bg-primary rounded-full" />
                <div className="flex-1">
                  <div className="flex justify-between text-xs font-bold text-foreground">
                    <span>Hot Edge Storage</span>
                    <span>1.2 TB</span>
                  </div>
                  <div className="text-[10px] text-muted-foreground mt-0.5">Instant access / Real-time processing</div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[10px] text-muted-foreground mt-0.5">Archived event clips / 7 day retention</div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[10px] text-muted-foreground mt-0.5">LTS Compliance data / 1 year retention</div>
                </div>
              </div>
            </div>
          ...
          <Card className="bg-muted/40 border border-border p-6 rounded-xl">
            <h3 className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Next Invoice</h3>
            <div className="flex items-baseline gap-1">
              <span className="text-xs text-muted-foreground">$</span>
              <span className="text-2xl font-bold text-foreground">{data?.nextInvoice.amount.replace("$", "")}</span>
            </div>
            <p className="text-[10px] text-muted-foreground mt-1">Estimated total for {data?.nextInvoice.period}</p>
      ...
      <div className="p-6 bg-surface border border-border rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm">
        <div className="flex items-start md:items-center gap-4">
          ...
          <div>
            <h4 className="font-bold text-foreground text-sm">Scale your operations effortlessly</h4>
            <p className="text-xs text-muted-foreground mt-0.5">Looking to add more branches? Upgrade to Enterprise Platinum for unlimited camera licenses.</p>
          </div>
        </div>
        <div className="flex gap-2 w-full md:w-auto shrink-0 justify-end">
          <Button variant="ghost" size="sm" className="font-bold">Compare Plans</Button>
          <Button size="sm" className="bg-orange-500 text-white hover:bg-orange-600 font-bold active:scale-95 transition-transform">Upgrade Plan</Button>
        </div>
      </div>
    </div>
  );
