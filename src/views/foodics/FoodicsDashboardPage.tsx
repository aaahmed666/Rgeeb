      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard icon={ShoppingCart} label={t("foodics.totalOrders")} value={String(s?.total_orders ?? 0)} bg="bg-gradient-to-br from-indigo-500 to-purple-600" />
            <StatCard icon={DollarSign} label={t("foodics.revenue")} value={`${fmt(s?.total_revenue ?? 0)}`} bg="bg-gradient-to-br from-emerald-500 to-teal-600" />
            <StatCard icon={Ticket} label={t("foodics.avgOrderValue")} value={`${fmt(s?.avg_ticket ?? null)}`} bg="bg-gradient-to-br from-sky-500 to-cyan-600" />
            <StatCard icon={ArrowRightLeft} label={t("foodics.conversion")}
              value={s?.conversion_rate != null ? `${fmt(s.conversion_rate)}%` : "—"}
              bg="bg-gradient-to-br from-amber-500 to-orange-600" />
          </div>

          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard icon={Receipt} label={t("foodics.refund")}
              value={String(s?.suspicious_refunds ?? 0)}
              sub={`${fmt(s?.suspicious_refunds_pct ?? 0, 1)}% of orders`}
              bg="bg-gradient-to-br from-rose-500 to-red-600" />
            <StatCard icon={CreditCard} label={t("foodics.cashDrawer")}
              value={String(s?.drawer_flags ?? 0)}
              sub={`${fmt(s?.drawer_flags_pct ?? 0, 1)}% of opens`}
              bg="bg-gradient-to-br from-orange-500 to-amber-600" />
            <StatCard icon={ChefHat} label={t("foodics.avgPrepTime")}
              value={s?.avg_kitchen_prep != null ? `${fmt(s.avg_kitchen_prep, 0)}m` : "—"}
              sub={`${fmt(s?.avg_kitchen_matched_pct ?? 0, 0)}% matched`}
              bg="bg-gradient-to-br from-violet-500 to-purple-600" />
            <StatCard icon={Package} label={t("foodics.inventory")}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="rounded-xl border border-border bg-card p-6">
              ...
              <div className="space-y-3">
                <TrendRow
                  icon={ShoppingCart} label={t("foodics.orders")} color="bg-indigo-500"
                  detail={`${data?.trends?.orders?.days ?? 0} days of data • ${data?.trends?.orders?.total ?? 0} total orders`}
                />
                <TrendRow
                  icon={ChefHat} label={t("foodics.prepTime")} color="bg-amber-500"
                ...
                />
                <TrendRow
                  icon={ArrowRightLeft} label={t("foodics.conversion")} color="bg-sky-500"
                  detail={`${data?.trends?.conversion?.days ?? 0} days • ${data?.trends?.conversion?.visitors ?? 0} total visitors`}
                />
                <TrendRow
                  icon={Receipt} label={t("foodics.refund")} color="bg-rose-500"
